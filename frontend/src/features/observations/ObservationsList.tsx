...
export const ObservationsList = () => {
  ...
  const handleEdit = (o: Observation) => {
    setSelected(o);
    setShowForm(true);
  };
  const handleView = (o: Observation) => {
    setSelected(o);
    setShowDetails(true);
  };

  // حماية من undefined/null
  const filtered = safeArray<Observation>(items);
        ...
        <Select onValueChange={(v) => setFilters((f) => ({ ...f, type: v }))}>
          ...
          <SelectContent>
            <SelectItem value="violation">
              {t("observations.types.violation")}
            </SelectItem>
            <SelectItem value="note">{t("observations.types.note")}</SelectItem>
            <SelectItem value="complaint">
              {t("observations.types.complaint")}
            </SelectItem>
          </SelectContent>
        </Select>
        <Select
          onValueChange={(v) => setFilters((f) => ({ ...f, committee_id: v }))}
        >
          <SelectTrigger className="w-[180px]">
          ...
          <thead>
            <tr className="text-left">
              ...
              <th className="px-4 py-2">
                {t("observations.observation_type")}
              </th>

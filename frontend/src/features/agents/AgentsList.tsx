...
class AgentsListErrorBoundary extends Component<{
  ...
  render() {
    ...
    if (hasError && error) {
      return <ErrorDisplay error={error} onRetry={this.handleRetry} title={title} />;
    }
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className="text-left">
                  ...
                  <th className="px-4 py-2">{t('agents.committee', { defaultValue: 'Committee' })}</th>
                  <th className="px-4 py-2">{t('common.actions')}</th>
                </tr>
              </thead>
              <tbody>
                {data.map((agent) => (
                  <tr key={agent.id} className="border-t border-white/10 hover:bg-accent/5 transition-colors">
                    <td className="px-4 py-3 font-medium">{agent.name}</td>
                    <td className="px-4 py-3 text-muted-foreground">{agent.mobile}</td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-1">
                        <Button size="sm" variant="ghost" onClick={() => onView(agent)} className="hover:bg-primary/10">
                          <Eye className="h-4 w-4" />
                        </Button>
                        <Button size="sm" variant="ghost" onClick={() => onEdit(agent)} className="hover:bg-secondary/10">
                          <Edit className="h-4 w-4" />
                        </Button>
                        <Button size="sm" variant="ghost" onClick={() => onAssign(agent)} className="hover:bg-accent/10">
...
export const AgentsList = () => {
  ...
  const handleEdit = (agent: Agent) => { setSelected(agent); setShowForm(true); };
  const handleView = (agent: Agent) => { setSelected(agent); setShowDetails(true); };
  const handleAssign = (agent: Agent) => { setSelected(agent); setShowAssign(true); };

  const handleDelete = async (id: string) => {
    try {
      await deleteAgent(id);
      toast({ title: t('common.delete'), description: t('agents.delete_success', { defaultValue: 'Agent deleted successfully' }) });
      refresh();
    } catch (err) {
      toast({ title: t('common.error'), description: t('agents.delete_error', { defaultValue: 'Failed to delete agent' }), variant: 'destructive' });
    }
  };
  ...
  const handleExport = useCallback(async () => {
    try {
      await exportAgents();
      toast({ title: t('agents.export_title', { defaultValue: 'Export' }), description: t('agents.export_success', { defaultValue: 'Agents exported successfully' }) });
    } catch (err) {
      toast({ title: t('common.error'), description: t('agents.export_error', { defaultValue: 'Failed to export agents' }), variant: 'destructive' });
    }
  }, [toast, t]);
  ...
  const onAssign = async (ids: string[]) => {
    if (selected && ids[0]) {
      try {
        ...
        toast({ title: t('agents.assignment_title', { defaultValue: 'Assignment' }), description: t('agents.assignment_success', { defaultValue: 'Agent assigned successfully' }) });
        setShowAssign(false);
        refresh();
      } catch (err) {
        toast({ title: t('common.error'), description: t('agents.assignment_error', { defaultValue: 'Failed to assign agent' }), variant: 'destructive' });
      }
    }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <AgentsListErrorBoundary
        onRetry={refresh}
        title={t('agents.load_error', { defaultValue: 'Failed to load agents' })}
      >
        <Suspense fallback={<AgentsListFallback message={t('agents.loading', { defaultValue: 'Loading agents...' })} />}>
          <AgentsListSuspended
            filters={filters}
            search={search}
            onView={handleView}
            ...
            onRefresh={refresh}
            exportLabel={t('agents.export_button', { defaultValue: 'Export agents' })}
          />
        </Suspense>
      </AgentsListErrorBoundary>
      ...
      <AgentDetails
        ...
        agent={selected}
        onEdit={agent => { setShowDetails(false); handleEdit(agent); }}
      />
      <AssignDialog
        isOpen={showAssign}

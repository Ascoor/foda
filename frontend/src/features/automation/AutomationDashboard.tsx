...
export const AutomationDashboard = () => {
  ...
  const handleToggle = async (taskName: string, isEnabled: boolean) => {
    ...
    try {
      ...
      if (updated) {
        setTasks((prev) =>
          prev.map((task) => (task.task === taskName ? { ...task, is_enabled: updated.is_enabled } : task))
        );
      }
  ...
  const handleTrigger = async (taskName: string) => {
    ...
    try {
      ...
      setTasks((prev) =>
        prev.map((task) => (task.task === taskName ? { ...task, ...updated } : task))
      );
  ...
  const orderedTasks = useMemo(() => {
    return [...tasks].sort((a, b) => a.display_name.localeCompare(b.display_name));
  }, [tasks]);

  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <div className="glass-card p-6">
        <h1 className="text-3xl font-bold text-gradient-primary">Automation Control Center</h1>
        <p className="mt-2 text-muted-foreground">
          Toggle campaign automation, review last execution details, and trigger urgent jobs on-demand.
        </p>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        {!loading && orderedTasks.map((task) => (
          <Card key={task.task} className={cn('border border-white/10 bg-background/60 backdrop-blur', !task.is_enabled && 'opacity-80')}>
            <CardHeader className="flex flex-row items-start justify-between space-y-0">
              <div>
                <CardTitle className="text-xl font-semibold">{task.display_name}</CardTitle>
                <CardDescription>{task.description}</CardDescription>
              </div>
              <Switch
                checked={task.is_enabled}
                onCheckedChange={(checked) => handleToggle(task.task, checked)}
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex items-center justify-between text-sm text-muted-foreground">
                <span>Status</span>
                <Badge variant={task.status === 'failed' ? 'destructive' : 'secondary'} className="capitalize">
                  {task.status ?? 'idle'}
                </Badge>
              </div>
              <div className="flex items-center justify-end gap-2">
                <Button
                  ...
                  disabled={activeTask === task.task}
                >
                  <RefreshCw className={cn('mr-2 h-4 w-4', activeTask === task.task && 'animate-spin')} />
                  Run now
                </Button>
              </div>
            </CardContent>

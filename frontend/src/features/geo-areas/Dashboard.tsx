...
export const GeoAreasDashboard = () => {
  ...
  const filtered = areas.filter((a) => {
    const matchesName = (language === "ar" ? a.name.ar : a.name.en)
      .toLowerCase()
  ...
  return (
    <div className="space-y-4" aria-label={t("navigation.geo_areas")}>
      ...
      <div className="flex flex-col md:flex-row gap-4">
        ...
        <Select
          value={status}
          onValueChange={(v) => setStatus(v as GeoStatus | "all")}
        >
          <SelectTrigger
            className="glass w-full md:w-48"
            aria-label={t("geo_areas.dashboard.status_filter")}
          >
        ...
        <div className="flex gap-2">
          ...
          >
            ...
            <span className="ml-2 hidden sm:inline">
              {t("geo_areas.dashboard.map_view")}
            </span>
          </Button>
          <Button
          ...
          >
            ...
            <span className="ml-2 hidden sm:inline">
              {t("geo_areas.dashboard.list_view")}
            </span>
          </Button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="relative h-96">
          ...
          {/* Legend */}
          <div
            className={`absolute bottom-4 ${direction === "rtl" ? "right-4" : "left-4"} bg-background/80 backdrop-blur-sm p-3 rounded-md shadow-md text-sm`}
            aria-label={t("geo_areas.dashboard.legend")}
          >
            <div className="font-medium mb-1">
              {t("geo_areas.dashboard.legend")}
            </div>
          ...
          {selectedArea && (
            <div
              className={`absolute top-0 ${direction === "rtl" ? "left-0" : "right-0"} w-full sm:w-80 h-full bg-background/90 backdrop-blur-md border ${direction === "rtl" ? "border-r" : "border-l"} border-white/10 p-4 overflow-y-auto`}
              aria-label={

...
const MapContainer = ({ 
  ...
  const getAreaColor = (area: GeoArea) => {
    switch (area.type) {
      case 'governorate': return 'bg-primary/20 border-primary';
      case 'district': return 'bg-secondary/20 border-secondary';
      case 'city': return 'bg-accent/20 border-accent';
      case 'village': return 'bg-success/20 border-success';
    ...
    <div className="relative w-full h-96 glass-card overflow-hidden">
            ...
            >
              <div className="flex items-center space-x-2">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs opacity-70">{area.total_voters} voters</p>
                </div>
              </div>
      ...
      {selectedArea && (
        ...
        >
          ...
          <p className="text-sm text-muted-foreground capitalize">{selectedArea.type}</p>
          <div className="mt-2 space-y-1 text-xs">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium">{selectedArea.total_committees}</span>
            </div>
          </div>
        </motion.div>
      )}
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {areaTypes.map(type => (
            <div key={type} className="flex items-center gap-2">
              <div className={`w-3 h-3 rounded border border-${typeColors[type as keyof typeof typeColors]}`} />

...
describe('design tokens', () => {
  ...
  it('keeps primary and secondary colors aligned between CSS and TypeScript maps', () => {
    const css = readCss()
    expect(css).toMatch(new RegExp(`--color-primary: ${COLOR_TOKENS.primary.hsl};`))
    expect(css).toMatch(new RegExp(`--color-secondary: ${COLOR_TOKENS.secondary.hsl};`))
    expect(css).toMatch(new RegExp(`--color-accent: ${COLOR_TOKENS.accent.hsl};`))
  })
})

...
export const NotificationDrawer = () => {
  ...
  return (
    <Sheet open={isDrawerOpen} onOpenChange={setDrawerOpen}>
      <SheetContent side="right" className="w-full sm:w-[420px] border-l border-white/10 bg-background/95 backdrop-blur">
        ...
        <div className="mt-4 flex items-center gap-2">
          {filterOptions.map((option) => (
            <Button
              ...
              className={cn('flex-1 capitalize', filter === option.value ? '' : 'text-muted-foreground')}
            ...
            {filteredNotifications.map((notification) => (
              <button
                ...
                className={cn(
                  ...
                  notification.read_at ? 'border-border' : 'border-primary/60 shadow-sm shadow-primary/20',
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <h3 className="mt-1 text-sm font-semibold leading-tight">{notification.title}</h3>
                  </div>
                  {!notification.read_at && <span className="h-2.5 w-2.5 rounded-full bg-primary" />}
                </div>
                ...
                <div className="mt-3 flex items-center justify-between text-xs text-muted-foreground/80">
                  <span>{notification.created_ago ?? new Date(notification.created_at).toLocaleString()}</span>
                  <span className="font-medium capitalize">Priority: {notification.priority}</span>
                </div>
              </button>
            ))}
          </div>

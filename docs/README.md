---
title: "Documentation Overview"
author: "Knowledge Management"
last_updated: "2024-05-19"
version: "v1.0"
status: "Active"
---

# Elections360 Documentation Overview

Welcome to the consolidated Elections360 documentation hub. The resources below are grouped by domain to match the shared structure.

## Architecture
- [System Overview](architecture/system-overview.md)
- [Data Lifecycle](architecture/data-lifecycle.md)
- [API Design](architecture/api-design.md)

## Frontend
- [UI Components](frontend/ui-components.md)
- [Feature Guides](frontend/feature-guides.md)
- [Routing Structure](frontend/routing-structure.md)
- [Frontend Audit Command Reference](frontend/frontend-audit-command-reference.md)

## Backend
- [Data Models](backend/data-models.md)
- [Endpoints Reference](backend/endpoints-reference.md)
- [Services Architecture](backend/services-architecture.md)

## Shared
- [Contexts and Hooks](shared/contexts-and-hooks.md)
- [Utilities and Types](shared/utilities-and-types.md)
- [Integrations](shared/integrations.md)

## Governance
- [Documentation Policy](governance/documentation-policy.md)
- [Versioning Guidelines](governance/versioning-guidelines.md)
- [Review Process](governance/review-process.md)

## Legacy
- [Deprecated Modules](legacy/deprecated-modules.md)
- [Migration Guide](legacy/migration-guide.md)

Additional assets:
- [Documentation Audit Report](Documentation_Audit_Report.md)
- [Removed Docs Log](Removed_Docs_Log.md)
- [`_index.yaml`](./_index.yaml) for machine-readable navigation.

Please follow the [Documentation Policy](governance/documentation-policy.md) before submitting updates.
